...
function analyzeVmMethod(sourceFile: ts.SourceFile, filePath: string): PropertyMapping[] {
...
function formatResults(results: { file: string; properties: PropertyMapping[] }[]): string {
  let output = '';
...
function main() {
  ...
  const totalRepeatedProps = results.reduce(
    (sum, result) => sum + result.properties.filter(prop => prop.isRepeat).length, 
    0
  );
  
  const filesWithRepeats = results.filter(
    result => result.properties.some(prop => prop.isRepeat)
  ).length;
